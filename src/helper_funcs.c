...
void toLower(char * word, uint8_t len){
    for(uint8_t t = 0; t < len; t++)
        if(word[t] >= 0x41 && word[t] <= 0x5A) word[t] += 0x20;
}

bool wordBeforeWord(char * word_new, char * word_old, uint8_t new_n, uint8_t old_n){
    uint8_t t = 0;
    char letter_new = word_new[t];
    char letter_old = word_old[t];
