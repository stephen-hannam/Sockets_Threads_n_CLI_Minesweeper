...
int main(int argc, char ** argv){
                ...
                for(uint8_t n = 0; n < MAX_PLAYERS; n++){
                    if(!active_request[n])
                        if(players[n].fd != NO_SOCKET  && FD_ISSET(players[n].fd, &read_fds)){
                            ...
                            // ioctl: guard against race conditions on read fds and spurious unblocks on pselect()
                            if(bytes_avail > 0){
    ...
    /* block SIGINT in worker threads and pass shutdown via single bool only modified by parent thread */
    sigint_thrds = true;
    /* CLEAN-UP */  
    
    // rejoin the threads - threads see sigint_recv by proxy
    for(uint8_t i = 0; i < NUM_THREADS; i++){
...
void handleReq(request_t * a_request){
    ...
    if(pc == CONN_OFF){
        fprintf(stderr, \
            "Player %d on socket %d has shutdown: no stream error, no notification.\n",\
             curr_id, curr_fd);
        close(curr_fd);
    ...
    else if(pc == CHIRP && packet[CHIRP_NAME] == '+'){ // resend packet as per chirp request from client
        ...
        printf("Player %d on socket %d advises shutting down from SIGINT.\n",\
            curr_id, curr_fd);
        close(curr_fd);
        a_request->player->fd = NO_SOCKET;
    }
        ...
        switch(packet[PCKT_VERB]){ // VERB
            ...
            // QUIT WHOLE GAME
            case 'Q': 
                printf("Player %d on socket %d advises shutting down normally.\n",\
                    curr_id, curr_fd);
...
int8_t listenOnSocket(int * sockfd, uint16_t port){
    ...
    *sockfd = socket(AF_INET, SOCK_STREAM, 0);
    ...
    my_addr.sin_addr.s_addr = htonl(INADDR_ANY); // byte order unaffected, 0x0 in 0x0 out, but better practice to use htonl anyway
